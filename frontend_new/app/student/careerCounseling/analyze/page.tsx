...
function CounselingDashboard() {
  ...
  const downloadTableAsCSV = () => {
    if (careerCounsels) {
      const headers = ["Year", "Name of the Activity", "Number of students attended", "Link to the relevant document"]
      const csvContent = [
        headers.join(","),
        ...careerCounsels.map((cc) => [cc.year, cc.activityName, cc.numberOfStudents, cc.documentLink].join(",")),
      ].join("\n")
                  ...
                  <PaginationContent>
                    ...
                    <PaginationItem>
                      <PaginationNext
                        onClick={() =>
                          setCurrentPage((prev) =>
                            Math.min(Math.ceil((careerCounsels?.length || 0) / itemsPerPage), prev + 1),
                          )
                        }
                      ...
                      <PaginationNext
                        onClick={() =>
                          setCardCurrentPage((prev) =>
                            Math.min(Math.ceil((careerCounsels?.length || 0) / cardsPerPage), prev + 1),
                          )
                        }
