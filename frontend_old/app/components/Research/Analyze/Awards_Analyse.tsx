...
export default function AwardsCompare() {
    const [allAwards, setAllAwards] = useState<AwardJSON[]>([]);
    const [year1, setYear1] = useState<string>('');
    const [year2, setYear2] = useState<string>('');
    const [awardsCount1, setAwardsCount1] = useState<{ [key: string]: number }>({});
    const [awardsCount2, setAwardsCount2] = useState<{ [key: string]: number }>({});
    const [selectedYearAwards, setSelectedYearAwards] = useState<AwardJSON[]>([]);
    ...
    const fetchAwards = async () => {
        try {
            const response = await axios.get('http://localhost:8080/awards');
    ...
    const handleYearComparison = () => {
        const awards1 = allAwards.filter(award => award.year === year1);
        const awards2 = allAwards.filter(award => award.year === year2);

        const count1 = countAwardsByCategory(awards1);
        const count2 = countAwardsByCategory(awards2);

        setAwardsCount1(count1);
        setAwardsCount2(count2);

        updateChartData(count1, count2);

        setSelectedYearAwards([...awards1, ...awards2]);
    }

    const countAwardsByCategory = (awards: AwardJSON[]) => {
    ...
    const handleDownloadGraph = useCallback(() => {
        if (ref.current) {
            ...
            link.download = "awards_comparison.png";
                    ...
                    <div className='overflow-auto rounded-lg shadow-lg hidden md:block mb-5'>
                        <table className='w-full text-black' ref={tableRef}>
                            ...
                            <tbody className='divide-y divide-gray-100'>
                                {Object.keys(awardsCount1).map(category => (
                                    <tr key={category}>
                                        <td className='p-3 text-sm text-gray-700 whitespace-nowrap'>{category}</td>
                                        <td className='p-3 text-sm text-gray-700 whitespace-nowrap'>{awardsCount1[category] || 0}</td>
                                        <td className='p-3 text-sm text-gray-700 whitespace-nowrap'>{awardsCount2[category] || 0}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                        ...
                        <table className='w-full text-black'>
                            <thead className='bg-gray-100 border-b-2 border-gray-200'>
                                <tr>
                                    ...
                                    <th className='p-3 text-sm font-semibold tracking-wide text-left'>Awardee Name</th>
                                    <th className='p-3 text-sm font-semibold tracking-wide text-left'>Awarding Agency</th>
                                    <th className='p-3 text-sm font-semibold tracking-wide text-left'>Category</th>
                                </tr>
                            </thead>
                            <tbody className='divide-y divide-gray-100'>
                                {selectedYearAwards.map((award, index) => (
                                    <tr key={index}>
                                        <td className='p-3 text-sm text-gray-700 whitespace-nowrap'>{award.year}</td>
                                        <td className='p-3 text-sm text-gray-700 whitespace-nowrap'>{award.title}</td>
                                        <td className='p-3 text-sm text-gray-700 whitespace-nowrap'>{award.awardeeName}</td>
                                        <td className='p-3 text-sm text-gray-700 whitespace-nowrap'>{award.awardingAgencyName}</td>
                                        <td className='p-3 text-sm text-gray-700 whitespace-nowrap'>{award.category}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
